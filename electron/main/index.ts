...
ipcMain.handle('search-cards', (_, { query, type, limit }) => dbHandlers.searchCards(query, type, limit))
...
ipcMain.handle('get-taxonomy-counts', () => dbHandlers.getGlossaryTaxonomyCounts())
ipcMain.handle('migrate-glossary-domain', (_, payload) => dbHandlers.migrateGlossaryDomain(payload))
ipcMain.handle('migrate-glossary-field', (_, payload) => dbHandlers.migrateGlossaryField(payload))
ipcMain.handle('get-orphaned-glossary-tags', (_, validTags) => dbHandlers.getOrphanedGlossaryTags(validTags))
ipcMain.handle('batch-migrate-glossary-tags', (_, mappings) => dbHandlers.batchMigrateGlossaryTags(mappings))
